...
DOCKER_TAG = unstable
DOCKER_IMAGE = Khaos-Labs/tkchain
TKCHAIN_DAEMON_BINARY = tkchaind
TKCHAIN_CLI_BINARY = tkchaincli
GO_MOD=GO111MODULE=on
BUILDDIR ?= $(CURDIR)/build
SIMAPP = ./app
LEDGER_ENABLED ?= true
HTTPS_GIT := https://github.com/Khaos-Labs/tkchain.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=tkchain \
...
build-tkchain-linux: go.sum
	GOOS=linux GOARCH=amd64 CGO_ENABLED=1 $(MAKE) build-tkchain

.PHONY: build build-tkchain build-tkchain-linux
...
docker-build:
	...
	docker rm tkchain || true
	# create a new container from the latest image
	docker create --name tkchain -t -i Khaos-Labs/tkchain:latest tkchain
	# move the binaries to the ./build directory
	mkdir -p ./build/
	docker cp tkchain:/usr/bin/tkchaind ./build/ ; \
	docker cp tkchain:/usr/bin/tkchaincli ./build/

docker-localnet:
	docker build -f ./networks/local/tkchainnode/Dockerfile . -t tkchaind/node
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(TKCHAIN_DAEMON_BINARY)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(TKCHAIN_DAEMON_BINARY)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(TKCHAIN_DAEMON_BINARY)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Jobs=4 -SimAppPkg=$(SIMAPP) -ExitOnFail -Genesis=${HOME}/.$(TKCHAIN_DAEMON_BINARY)/config/genesis.json 400 5 TestFullAppSimulation
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -name '*.pb.go' | xargs goimports -w -local github.com/Khaos-Labs/tkchain
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(TKCHAIN_DAEMON_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\tkchain\Z tkchaind/node "tkchaind testnet --v 4 -o /tkchain --starting-ip-address 192.168.10.2 --keyring-backend=test"
	docker-compose up -d
else
	mkdir -p ./build/
	@$(MAKE) docker-localnet

	if ! [ -f build/node0/$(TKCHAIN_DAEMON_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/tkchain:Z tkchaind/node "tkchaind testnet --v 4 -o /tkchain --starting-ip-address 192.168.10.2 --keyring-backend=test"; fi
	docker-compose up -d
endif
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)/build\tkchain\Z tkchaind/node "tkchaind unsafe-reset-all --home=/tkchain/node0/tkchaind"
	@docker run --rm -v $(CURDIR)/build\tkchain\Z tkchaind/node "tkchaind unsafe-reset-all --home=/tkchain/node1/tkchaind"
	@docker run --rm -v $(CURDIR)/build\tkchain\Z tkchaind/node "tkchaind unsafe-reset-all --home=/tkchain/node2/tkchaind"
	@docker run --rm -v $(CURDIR)/build\tkchain\Z tkchaind/node "tkchaind unsafe-reset-all --home=/tkchain/node3/tkchaind"
else
	@docker run --rm -v $(CURDIR)/build:/tkchain:Z tkchaind/node "tkchaind unsafe-reset-all --home=/tkchain/node0/tkchaind"
	@docker run --rm -v $(CURDIR)/build:/tkchain:Z tkchaind/node "tkchaind unsafe-reset-all --home=/tkchain/node1/tkchaind"
	@docker run --rm -v $(CURDIR)/build:/tkchain:Z tkchaind/node "tkchaind unsafe-reset-all --home=/tkchain/node2/tkchaind"
	@docker run --rm -v $(CURDIR)/build:/tkchain:Z tkchaind/node "tkchaind unsafe-reset-all --home=/tkchain/node3/tkchaind"
endif

.PHONY: build-docker-local-tkchain localnet-start localnet-stop
