...
import (
	...
	"github.com/Khaos-Labs/tkchain/crypto/hd"
	ethermint "github.com/Khaos-Labs/tkchain/types"
	evmtypes "github.com/Khaos-Labs/tkchain/x/evm/types"
)
...
func TestnetCmd(ctx *server.Context, cdc *codec.Codec,
	...
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a Tkchain testnet",
		Long: `testnet will create "v" number of directories and populate each with
necessary files (private validator, genesis, config, etc.).

Note, strict routability for addresses is turned off in the config file.`,

		Example: "tkchain testnet --v 4 --keyring-backend test --output-dir ./output --starting-ip-address 192.168.10.2",
	...
	cmd.Flags().String(flagNodeDaemonHome, "tkchaind", "Home directory of the node's daemon configuration")
	cmd.Flags().String(flagNodeCLIHome, "tkchaincli", "Home directory of the node's cli configuration")
	...
	cmd.Flags().String(server.FlagMinGasPrices, fmt.Sprintf("0.000006%s", ethermint.AttoPhoton), "Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.01tkb,0.001tkb)")
...
func InitTestnet(
	...
	if chainID == "" {
		chainID = fmt.Sprintf("tkchain-%d", tmrand.Int63n(9999999999999)+1)
	}
